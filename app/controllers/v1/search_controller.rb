class V1::SearchController < ApplicationController
  def index
    ...
    person_includes = [:profile_image_attachment, :schools, :address, {
      taggings: [:tag], school_relationships: [school: [:taggings]]
    }]
    person_serialization_options = { include: %i[schools school_relationships] }
    if search_params[:profile_image_width]
      person_serialization_options[:params] = {
        profile_image_width: search_params[:profile_image_width]
      }
    end
    ...
    when 'person', 'people', 'persons'
      ...
      @results = @search.to_a
      render json: V1::PersonSerializer.new(@results, person_serialization_options)
    when 'school', 'schools'
