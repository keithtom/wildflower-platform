...
RSpec.describe V1::SSJ::TeamsController, type: :request do
  ...
  let(:etl_params_controller) do
    ActionController::Parameters.new({ team: { etl_people_params: } }).require(:team).permit([etl_people_params: %i[
  ...
  describe 'POST #create' do
    ...
    context 'when an admin makes the request' do
      ...
      context 'when the team is successfully invited' do
        let(:team) { create(:ssj_team, ops_guide:, regional_growth_lead: rgl) }

        it 'returns a success message' do
          post '/v1/ssj/teams',
               params: { team: { workflow_id: workflow_definition.id, ops_guide_id: ops_guide.external_identifier, rgl_id: rgl.external_identifier, etl_people_params: } }, headers: headers
    ...
    context 'when a non-admin makes the request' do
      ...
      it 'returns an unauthorized error message' do
        post '/v1/ssj/teams',
             params: { team: { ops_guide_id: ops_guide.external_identifier, rgl_id: rgl.external_identifier, etl_people_params: } }, headers: headers
  ...
  context 'existing team' do
    ...
    before do
      ...
      team = SSJ::Team.new(expected_start_date:,
                           workflow_id: create(:workflow_instance_workflow).id)
      team.save!
    ...
    describe 'GET #show' do
      it 'succeeds' do
        ...
        expect(json_response['data']['attributes']['expectedStartDate']).to eq(expected_start_date.to_fs('yyyy-mm-dd'))
      end
    end
    ...
    describe 'PUT #update' do
      ...
      it 'succeeds' do
        put "/v1/ssj/teams/#{team.external_identifier}", headers: headers,
                                                         params: { team: { expected_start_date: new_start_date } }
        expect(response).to have_http_status(:success)
        ...
        expect(ssj_team.reload.expected_start_date.to_fs('yyyy-mm-dd')).to eq(new_start_date)
      end
    end
    ...
    describe 'PUT /v1/ssj/teams/[team_id]/invite_partner' do
      ...
      it 'succeeds' do
        put "/v1/ssj/teams/#{team.external_identifier}/invite_partner", headers: headers, params: {
          person: {
            email:, first_name: Faker::Name.first_name, last_name: Faker::Name.last_name, primary_language: 'English',
        ...
        expect(response).to have_http_status(:success)
        expect(json_response['data']['attributes']['hasPartner']).to be(true)
        person = user.person
        team = person.ssj_team
        expect(SSJ::TeamMember.where(ssj_team_id: team.id, status: SSJ::TeamMember::INVITED)).not_to be_empty
      end
    end
  end
end
